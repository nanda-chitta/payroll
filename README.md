...
| Deployed software | In progress | Render blueprint defines separate backend and frontend services; add both live URLs after sync completes |
...
- The React app is deployed as a separate static site named `payroll-web`. Use that frontend URL for the UI. The Rails `payroll` service URL is the API/backend endpoint and will not render `/salary-management` by itself.
- The current blueprint points `VITE_API_BASE_URL` at `https://payroll-vgfr.onrender.com`. Update that value if your backend Render URL changes.

After both services are healthy, add the live frontend URL and backend URL to this README.
