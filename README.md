...
| Deployed software | Implemented | Live Render app: `https://payroll-vgfr.onrender.com/salary-management` |
...
Frontend and API are deployed on Render and served from the same app:

- App URL: `https://payroll-vgfr.onrender.com/salary-management`
- Health check: `https://payroll-vgfr.onrender.com/up`
...
- Set `DATABASE_URL` on the `payroll` web service to the Render Postgres connection string, or wire it from an existing Postgres database in the dashboard.
...
- `SECRET_KEY_BASE` is generated by the blueprint, but it can replace with the fixed secret in the Render dashboard if preferred.
...
```text
https://payroll-vgfr.onrender.com/salary-management
```
